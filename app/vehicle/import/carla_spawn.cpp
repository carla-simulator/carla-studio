...
DeployResult deployVehicleToShippingCarla(const VehicleRegistration &reg) {
  ...
  QString cookedContentRoot = copyFromDir;
  {
    ...
    const int idx = cookedContentRoot.lastIndexOf(needle);
    if (idx >= 0)
...
DriveTestResult driveTestVehicle(const QString &make, const QString &model,
  ...
  try {
    ...
    r.displacementM = std::sqrt(dx * dx + dy * dy) / 100.f;
    r.drivesForward = r.displacementM > 1.f;

    const carla::geom::BoundingBox bbox = vehicle->GetBoundingBox();
    r.bboxExtentX = bbox.extent.x;
    r.bboxExtentY = bbox.extent.y;
    r.bboxExtentZ = bbox.extent.z;
    constexpr float kMustangX = 220.f, kMustangY = 90.f, kMustangZ = 65.f;
    constexpr float kTol = 0.25f;
    r.bboxIsCustom = (std::abs(r.bboxExtentX - kMustangX) / kMustangX > kTol)
                  || (std::abs(r.bboxExtentY - kMustangY) / kMustangY > kTol)
                  || (std::abs(r.bboxExtentZ - kMustangZ) / kMustangZ > kTol);

    try {
      r.destroyClean = vehicle->Destroy();
    } catch (...) {
      r.destroyClean = false;
    }

    r.detail = QString("disp:%1m  bbox:%2x%3x%4cm  drives:%5  custom_bbox:%6  destroy:%7")
                 .arg(r.displacementM, 0, 'f', 2)
                 .arg(r.bboxExtentX * 2.f, 0, 'f', 0)
                 .arg(r.bboxExtentY * 2.f, 0, 'f', 0)
                 .arg(r.bboxExtentZ * 2.f, 0, 'f', 0)
                 .arg(r.drivesForward ? "YES" : "NO")
                 .arg(r.bboxIsCustom  ? "YES" : "NO")
                 .arg(r.destroyClean  ? "OK"  : "FAIL");
    return r;
  } catch (const std::exception &e) {
