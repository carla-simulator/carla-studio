...
int main(int argc, char **argv)
{
  ...
  // Optional wheel positions (cm) from the spec, so the calibration window
  // can render real markers at the actual anchors instead of leaving the
  // default placeholders below the floor.
  ...
  if (interactive) {
    ...
    win->resize(1024, 720);
    if (gotWheels) {
      win->showFor(meshPath, wheelFL, wheelFR, wheelRL, wheelRR);
    } else {
