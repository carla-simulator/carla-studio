...
void ViewGizmo::bindCamera(Qt3DRender::QCamera *cam)
{
  if (!cam) return;
  auto sync = [this, cam]() {
    mCamPos    = cam->position();
    mCamCenter = cam->viewCenter();
    mCamUp     = cam->upVector();
    reproject();
    redraw();
  };
...
{
  QVector3D dir = (mCamCenter - mCamPos).normalized();
  if (dir.isNull()) dir = QVector3D(0, -1, 0);
  QVector3D right = QVector3D::crossProduct(dir, mCamUp).normalized();
  ...
  for (auto &h : mHandles) {
    ...
    h.screenPt = { int(cx + R * sx), int(cy - R * sy) };
  }
}
...
void ViewGizmo::redraw()
{
  ...
  // Sort handles back-to-front
  std::array<int, 6> order = {0, 1, 2, 3, 4, 5};
  std::sort(order.begin(), order.end(), [this](int a, int b) {
    return mHandles[a].depth < mHandles[b].depth;
  });

  const int dotR = 13;
  for (int idx : order) {
    const Handle &h = mHandles[idx];
    ...
    if (!back) {
      ...
      p.drawEllipse(QPoint(h.screenPt.x() + 2, h.screenPt.y() + 2), r, r);
    }
    ...
    const QRect tr(h.screenPt.x() - r, h.screenPt.y() - r, r * 2, r * 2);
...
void ViewGizmo::mousePressEvent(QMouseEvent *ev)
{
  ...
  int   bestI = -1;
  for (int i = 0; i < 6; ++i) {
    const float dx = float(pt.x() - mHandles[i].screenPt.x());
    const float dy = float(pt.y() - mHandles[i].screenPt.y());
  ...
  emit snapRequested(snapPos[bestI], QVector3D(0, 0, 80), snapUp[bestI]);
}
