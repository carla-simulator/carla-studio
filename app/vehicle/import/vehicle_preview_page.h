...
class VehiclePreviewPage : public QWidget {
 ...
 private:
  ...
  void buildAlignmentRods(Qt3DCore::QEntity *root);
  void updateAlignmentLines();
  void updateMeshBounds();
  void applyAdjustment(const QString &label);
  ...
  bool                                mWheelPositionsExternal = false;
  ...
  // 5=body-front  6-9=tire-forward-arrows (FL FR RL RR)
  std::array<AlignRod, 10> mAlignRods{};
  QVector3D                mLastWheelPos[4]{};

  ToolMode        mToolMode       = ToolMode::Import;
  ViewGizmo      *mViewGizmo      = nullptr;
  QStackedWidget *mLeftStack      = nullptr;
  QStackedWidget *mRightStack     = nullptr;
  QPushButton    *mImportModeBtn  = nullptr;
  QPushButton    *mSensorModeBtn  = nullptr;
