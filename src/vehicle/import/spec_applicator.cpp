...
#include "vehicle/import/spec_applicator.h"

namespace carla_studio::vehicle_import {

ApplyResult apply_spec_to_editor(const VehicleSpec &spec,
                              const QString & ) {
  ApplyResult r;
  r.bp_asset_path = QStringLiteral("/Game/Carla/Static/Vehicles/4Wheeled/")
...
ValidationReport validate_imported_vehicle(const QString &bp_asset_path,
  ...
  v.detail = QStringLiteral(
      ...
      "(a) spawn `") + bp_asset_path + QStringLiteral("_C` as a transient actor in the editor world; "
