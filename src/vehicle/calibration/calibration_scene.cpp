...
QJsonObject PlacedTarget::to_json() const {
  QJsonObject o;
...
static QString persistDir(const QString &scenario_id) {
  ...
  return d.filePath(safeKey(scenario_id));
}

static QString persistFile(const QString &scenario_id,
                           const QString &sensor_name) {
  QDir d(persistDir(scenario_id));
  if (!d.exists()) d.mkpath(".");
  return d.filePath(safeKey(sensor_name) + ".json");
}

QList<PlacedTarget> load_persisted(const QString &scenario_id,
                                  const QString &sensor_name) {
  QList<PlacedTarget> out;
  QFile f(persistFile(scenario_id, sensor_name));
  if (!f.open(QIODevice::ReadOnly)) return out;
  const QJsonDocument doc = QJsonDocument::fromJson(f.readAll());
  if (!doc.isObject()) return out;
  const QJsonArray arr = doc.object()["targets"].toArray();
  for (const QJsonValue &v : arr)
    if (v.isObject()) out.append(PlacedTarget::from_json(v.toObject()));
  return out;
}
...
void save_persisted(const QString &scenario_id,
  ...
  QJsonArray arr;
  for (const PlacedTarget &t : targets) arr.append(t.to_json());
  QJsonObject root;
...
static QVector3D scaleForType(TargetType t, const PlacedTarget &p) {
  const float s = static_cast<float>(p.size_m);
  switch (t) {
...
CalibrationScene::CalibrationScene(QWidget *parent)
  ...
  m_view->defaultFrameGraph()->setClearColor(QColor("#1a1f2c"));
  ...
  auto *fillLightEntity = new Qt3DCore::QEntity(m_root);
  ...
  m_ground = new Qt3DCore::QEntity(m_root);
  ...
  m_ground->addComponent(groundMesh);
  m_ground->addComponent(groundMat);
  m_ground->addComponent(groundXform);

  auto *groundPicker = new Qt3DRender::QObjectPicker(m_ground);
  groundPicker->setHoverEnabled(false);
  m_ground->addComponent(groundPicker);
  connect(groundPicker, &Qt3DRender::QObjectPicker::clicked,
          this, &CalibrationScene::on_ground_clicked);

  for (int i = -20; i <= 20; i += 5) {
    auto *line = new Qt3DCore::QEntity(m_root);
  ...
  m_hero = new Qt3DCore::QEntity(m_root);
  m_hero_xform = new Qt3DCore::QTransform();
  m_hero_xform->setTranslation(QVector3D(0.0f, 0.0f, 0.0f));
  m_hero->addComponent(m_hero_xform);
  ...
  {
    auto *e = new Qt3DCore::QEntity(m_hero);
  ...
  m_sensor_marker = new Qt3DCore::QEntity(m_root);
  ...
  m_sensor_xform = new Qt3DCore::QTransform();
  m_sensor_xform->setTranslation(QVector3D(1.0f, 1.5f, 0.0f));
  m_sensor_marker->addComponent(sMesh);
  m_sensor_marker->addComponent(sMat);
  m_sensor_marker->addComponent(m_sensor_xform);

  m_targets_root = new Qt3DCore::QEntity(m_root);

  m_view->renderSettings()->pickingSettings()->setPickMethod(
      Qt3DRender::QPickingSettings::TrianglePicking);
  m_view->renderSettings()->pickingSettings()->setPickResultMode(
      Qt3DRender::QPickingSettings::NearestPick);
}

CalibrationScene::~CalibrationScene() = default;

void CalibrationScene::set_hero_pose(double xMeters, double yMeters, double yaw_deg) {
  m_hero_xform->setTranslation(QVector3D(static_cast<float>(yMeters), 0.75f,
                                       static_cast<float>(-xMeters)));
  m_hero_xform->setRotationY(static_cast<float>(-yaw_deg));
}

void CalibrationScene::set_sensor_pose(double xMeters, double yMeters,
                                     double zMeters, double yaw_deg) {
  m_sensor_xform->setTranslation(QVector3D(static_cast<float>(yMeters),
                                         static_cast<float>(zMeters + 0.05),
                                         static_cast<float>(-xMeters)));
  m_sensor_xform->setRotationY(static_cast<float>(-yaw_deg));
}

void CalibrationScene::set_targets(const QList<PlacedTarget> &targets) {
  m_targets = targets;
  m_selected = -1;
  rebuild_target_entities();
  emit targets_changed(m_targets);
  emit selection_changed(m_selected);
}

void CalibrationScene::add_target_at_center(TargetType t) {
  PlacedTarget p;
  p.type = t;
  p.x = 5.0;
  p.y = 0.0;
...
void CalibrationScene::add_target_at(TargetType t, double xWorld, double yWorld) {
  PlacedTarget p;
  p.type = t;
  p.x = xWorld;
  p.y = yWorld;
...
void CalibrationScene::update_target_pose(int index, double x, double y, double z, double yaw_deg) {
  if (index < 0 || index >= m_targets.size()) return;
  m_targets[index].x      = x;
  m_targets[index].y      = y;
  m_targets[index].z      = z;
  m_targets[index].yaw_deg = yaw_deg;
  rebuild_target_entities();
  emit targets_changed(m_targets);
}

void CalibrationScene::remove_at(int index) {
  if (index < 0 || index >= m_targets.size()) return;
...
void CalibrationScene::clear_all() {
  m_targets.clear();
  m_selected = -1;
  rebuild_target_entities();
  emit targets_changed(m_targets);
...
void CalibrationScene::rebuild_target_entities() {
  const auto kids = m_targets_root->children();
  for (QObject *c : kids) c->deleteLater();

  for (int i = 0; i < m_targets.size(); ++i) {
    const PlacedTarget &p = m_targets[i];
    auto *ent = new Qt3DCore::QEntity(m_targets_root);
    ...
    if (i == m_selected) c = c.lighter(140);
    mat->setDiffuse(c);
    mat->setAmbient(c);
    ...
    xf->setRotationY(static_cast<float>(-p.yaw_deg));
...
void CalibrationScene::on_target_clicked(int index, Qt3DRender::QPickEvent *) {
  m_selected = index;
  rebuild_target_entities();
  emit selection_changed(m_selected);
}
